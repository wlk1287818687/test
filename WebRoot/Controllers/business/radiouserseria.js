...
xh.load = function() {
	...
	app.controller("radiouserseria", function($scope, $http) {
		xh.maskShow();
		$scope.count = "20";// 每页数据显示默认值
		$scope.systemMenu = true; // 菜单变色
		$http.get(
				"../../radiouserseria/list?id=" + id + "&name=" + name
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get(
					"../../radiouserseria/list?id=" + id + "&name=" + name + "&start="
...
// 刷新数据
xh.refresh = function() {
	var appElement = document.querySelector('[ng-controller=radiouserseria]');
